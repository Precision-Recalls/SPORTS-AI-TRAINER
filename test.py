...
model = YOLO("/Users/garvagarwal/Desktop/SPORTS-AI-TRAINER/best_yolo8s.pt")
...
def display_angles(frame, angles):
    ...
    if "left_elbow" in angles:
        text, position, font_scale, thickness = scale_text(frame, f"Left Elbow Angle: {angles['left_elbow']:.2f}", (10,60), 1, 2)
        cv2.putText(frame, text, position, cv2.FONT_HERSHEY_SIMPLEX, font_scale, (0,0,0), thickness)
           
    
    if "right_elbow" in angles:
        text, position, font_scale, thickness = scale_text(frame, f"Right Elbow Angle: {angles['right_elbow']:.2f}", (10,90), 1, 2)
        cv2.putText(frame, text, position, cv2.FONT_HERSHEY_SIMPLEX, font_scale, (0, 0, 0), thickness)
...
def track_ball(predictions, frame):
    ...
    for bbox in predictions:
        ...
        if model.names[int(cls)] == ball_label:
            ...
            # Scale text
            text, position, font_scale, thickness = scale_text(frame, f"Ball Coordinates: {ball_center}", (10,120), 1, 2)
            cv2.putText(frame, text, position, cv2.FONT_HERSHEY_SIMPLEX, font_scale, (0, 0, 0), thickness)
            break
...
def detect_shot_and_goal(ball_positions, basket_bbox, frame):
    ...
    for pos in ball_positions:
        if np.linalg.norm(np.array(pos) - np.array(basket_center)) <= proximity_radius and shot_frame_counter > min_frames_between_shots:
            shot_detected = True
            shot_counter += 1
            shot_frame_counter = 0
            #text, position, font_scale, thickness = scale_text(frame, f"Shot Detected! Count: {shot_counter}", (10, 150), 1, 3)
            #cv2.putText(frame, text, position, cv2.FONT_HERSHEY_SIMPLEX, font_scale, (0, 255, 255), thickness)
            break

    if shot_detected:
        basket_top = basket_bbox[1] - 10
        basket_bottom = basket_bbox[3] + 10
        for i in range(1, len(ball_positions)):
            if ball_positions[i - 1][1] < basket_top and ball_positions[i][1] > basket_bottom and goal_frame_counter > min_frames_between_goals:
                goal_counter += 1
                goal_frame_counter = 0
